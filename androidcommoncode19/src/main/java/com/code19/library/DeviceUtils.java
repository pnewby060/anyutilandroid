...
public class DeviceUtils {
     ...
     * 是随机生成的设备的第一个引导，其记录着一个固定值，通过它可以知道设备的寿命（在设备恢复出厂设置后，该值可能会改变）。 ANDROID_ID也可视为作为唯一设备标识号的一个好选择。如要检索用于设备ID 的ANDROID_ID，请参阅下面的示例代码
     ...
     * 序列号可以用于识别MID（移动互联网设备）或PMP（便携式媒体播放器），这两种设备都没有电话功能。
     * 通过读取系统属性值“ro.serialno”的方法，可以使用序列号作为设备ID 。如检索序列号并作为设备ID使用，
     * 请参考下面的代码示例。
