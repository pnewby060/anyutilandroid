...
public class StringUtils {
	...
	public static int compare(String string1, String string2) {
		...
		while(char1Index < chars1.length && char2Index < chars2.length){
			char[] charss1 = getChars(chars1, char1Index);
			char1Index += charss1.length;
			char[] charss2 = getChars(chars2, char2Index);
			...
			if(charss1.length == 1 && charss2.length == 1){
				...
				boolean chars1AllDigital = isAllDigital(charss1);
				boolean chars2AllDigital = isAllDigital(charss2);
				//如果两个字符都是数字
	...
	public static String valueOf(byte[] bytes, Charset charset){
		try {
			return new String(IOUtils.read(new InputStreamReader(new ByteArrayInputStream(bytes), charset)));
		} catch (IOException e) {
			e.printStackTrace();
			return null;
		}
	...
	public static String valueOf(byte[] bytes, String charsetName){
		return valueOf(bytes, Charset.forName(charsetName));
	}
	...
	public static String valueOf(byte[] bytes, int offSet, int length, Charset charset){
		try {
			return new String(IOUtils.read(new InputStreamReader(new ByteArrayInputStream(bytes, offSet, length), charset)));
		} catch (IOException e) {
			e.printStackTrace();
			return null;
		}
	 ...
	 * @return 是否符合要求
	 */
	public static boolean checkUp(String string, List<String> listKeywords, StringVerifyMode checkUpWay){
		...
		if(checkUpWay == StringVerifyMode.CONTAIN_KEYWORDS){
			...
			result = !checkUp(string, listKeywords, StringVerifyMode.CONTAIN_KEYWORDS);
			...
			result = !checkUp(string, listKeywords, StringVerifyMode.EQUAL_KEYWORDS);
			...
			result = !checkUp(string, listKeywords, StringVerifyMode.ENDS_WITH_KEYWORDS);
			...
			result = !checkUp(string, listKeywords, StringVerifyMode.STARTS_WIT_KEYWORDS);
		}
		return result;
	}
